...
const STEPS = [
  ...
  },
  {
    ...
    body: (
      <>
        실전투자를 이용하려면 먼저 <strong>계정 설정에서 Upbit API Key를 등록</strong>하고
        연결 상태가 정상인지 확인해야 합니다. 그다음 <strong>"실전투자" 토글을 켜야</strong>{' '}
        실제 주문이 실행됩니다. 토글이 꺼져 있으면 실제 매매는 나가지 않습니다. 모의투자로
        충분히 검증한 전략이 있다면 API Key 연결과 토글 활성화를 마친 뒤, 실전투자 화면에서
        전략을 구독해 자동매매를 시작하세요.
      </>
    ),
    ctas: [
      { label: 'Upbit API Key 등록하기', to: '/settings' },
