...
function PnlChart({ points }) {
  ...
  return <div className={styles.lineChart}><div className={styles.chartValue}><strong className={positive ? styles.up : styles.down}>{won(values.at(-1))}</strong><span>누적</span></div><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="30일 누적 실현손익 그래프"><line x1="0" y1={zeroY} x2="100" y2={zeroY} className={styles.zeroLine} /><polyline points={coords} className={positive ? styles.positiveLine : styles.negativeLine} /></svg><div className={styles.axis}><span>{points[0]?.date?.slice(5)}</span><span>{points.at(-1)?.date?.slice(5)}</span></div></div>
}
